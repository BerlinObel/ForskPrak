...
def bulk_test(elements):
    ...
    for j in range(1):
        ...
        ax.text(0.02, 0.98,r'N$_{elements}$: '+f'{len(elements)};'+ f" NP size: {2160} "+ f"\nPeridocity:{pbc}"+f'\nShape:{k}\n'+f'Scale:{theta}'+f'\nShape: {k2}'  , family='monospace', fontsize=13, transform=ax.transAxes,verticalalignment='top')
        
        ax.set_xlabel(r"$\chi^2$", fontsize=16)
        ax.set_ylabel('Frequency', fontsize=16)
        ax2.set_ylabel('Probability', fontsize=16)

        fig.savefig(f'pvals/bulk_{len(elements)}_{2160}_{pbc}t.png')

        np.savetxt(f"bulk_{len(elements)}_{2160}_{pbc}t",pval_bulk)
