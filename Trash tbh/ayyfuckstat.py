...
def pearsons_chi2(observed_N, expected_N, ratio):
    chi2 = (np.sum((observed_N - expected_N) ** 2 / expected_N))/ratio
...
for kwargs in ParameterGrid(kwarg_grid):
        ...
        for i in range(N_particles):
            ...
            atoms = grid_particle(kwargs['elements'],5,250,kwargs['n_hops'],1.0,kwargs['het_mod'],0.0,i,False)
            #dub
            atomsdub = grid_particle(kwargs['elements'],5,500,kwargs['n_hops'],1.0,kwargs['het_mod'],0.0,i,True)
            ...
            ratio = np.average(dubobserved/observed)
            
            
            a1, b1, pval = pearsons_chi2(observed, expected, 1)
            a4, b4, pval4 = pearsons_chi2(dubobserved, dubexpected,1)
            a2, b2, pval2 = pearsons_chi2(dubobserved, dubexpected,ratio)
            a3, b3, pval3 = pearsons_chi2(dubobserved, dubexpected, 2)
           
                        
            ratios.append(ratio)
            pval_bootstrap.append(pval)
            pval2_bootstrap.append(pval2)
            pvaltest_bootstrap.append(pval3)
            pvalbig.append(pval4)
        ...
        f'\nMedian p-value = {np.median(pval_bootstrap):.2f}', family='monospace', fontsize=13, transform=ax[0].transAxes,verticalalignment='top')
        ax[0].set_xlabel(r"Pearson's $\chi^2$ standard", fontsize=16)
        ax[0].set_ylabel('Frequency', fontsize=16)
        #
        ax[1].hist(pvalbig, bins=25, range=(0, 1), histtype='bar', color='steelblue', alpha=0.7)
        ax[1].hist(pvalbig, bins=25, range=(0, 1), histtype='step', color='steelblue')
        ax[1].vlines(np.median(pvalbig), 0, ax[1].get_ylim()[1], color='firebrick')
        ax[1].set(ylim=(0, ax[1].get_ylim()[1] * 1.2))
        ax[1].text(0.02, 0.98,f'Median p-value = {np.median(pvalbig):.2f}', family='monospace', fontsize=13, transform=ax[1].transAxes,verticalalignment='top')
        ax[1].set_xlabel(r"Pearson's $\chi^2$ Large NP", fontsize=16)
        #
        ax[2].hist(pvaltest_bootstrap, bins=25, range=(0, 1), histtype='bar', color='steelblue', alpha=0.7)
        ax[2].hist(pvaltest_bootstrap, bins=25, range=(0, 1), histtype='step', color='steelblue')
        ax[2].vlines(np.median(pvaltest_bootstrap), 0, ax[2].get_ylim()[1], color='firebrick')
        ax[2].set(ylim=(0, ax[2].get_ylim()[1] * 1.2))
        ax[2].text(0.02, 0.98,f'Median p-value = {np.median(pvaltest_bootstrap):.2f}', family='monospace', fontsize=13, transform=ax[2].transAxes,verticalalignment='top')
        ax[2].set_xlabel(r"NP-size adjusted", fontsize=16)
        #
        ax[3].hist(pval2_bootstrap, bins=25, range=(0, 1), histtype='bar', color='steelblue', alpha=0.7)
        ax[3].hist(pval2_bootstrap, bins=25, range=(0, 1), histtype='step', color='steelblue')
        ax[3].vlines(np.median(pval2_bootstrap), 0, ax[1].get_ylim()[1], color='firebrick')
        ax[3].set(ylim=(0, ax[3].get_ylim()[1] * 1.2))
        ax[3].text(0.02, 0.98,f'Median p-value = {np.median(pval2_bootstrap):.2f}'+f'\nMedian bond ratio:{np.median(ratios)}', family='monospace', fontsize=13, transform=ax[3].transAxes,verticalalignment='top')
        ax[3].set_xlabel(r"Bonds amount adjusted ", fontsize=16)
        #
        fig.tight_layout()
